...
- **是否需要重新送审，取决于换没换构建**：
  ...
  - **换了构建（2026-09 实测）**：版本会回到「准备提交」状态，**回复不够**——必须去版本页点顶部「Update Review」（把更新后的版本重新装进提审单，提审条目变 Ready for Review）→ 回到提审详情页点「Resubmit to App Review」→ 状态变 Waiting for Review 才算真正送审。漏掉这步 = App 静静躺在「准备提交」没人审。
